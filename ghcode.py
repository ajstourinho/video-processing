...
FRAME_RATE = 30                   # no.of frames per second that needs to be processed, fewer the count faster the speed
WARMUP = 0             # initial number of frames to be skipped
FGBG_HISTORY = FRAME_RATE * 15   # no.of frames in background object
VAR_THRESHOLD = 64              # Threshold on the squared Mahalanobis distance between the pixel and the model to decide whether a pixel is well described by the background model.
DETECT_SHADOWS = False            # If true, the algorithm will detect shadows and mark them.
MIN_PERCENT = 0.05                # min % of diff between foreground and background to detect if motion has stopped
MAX_PERCENT = 5                  # max % of diff between foreground and background to detect if frame is still in motion
...
#            mask = cv2.dilate(mask, None, iterations=2)
        ...
        if p_diff < MIN_PERCENT and not captured and frame_count > WARMUP:
            ...
            print(f'Timestamp of frame: {int(timestamp // 3600):02d}:{int((timestamp % 3600) // 60):02d}:{int(timestamp % 60):02d}')
